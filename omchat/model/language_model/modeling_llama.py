...
class LlamaRotaryEmbedding(nn.Module):
    def __init__(self, dim, max_position_embeddings=2048, base=10000, device=None, use_fused_rotary=False):
        super().__init__()
        self.dim = dim
        ...
        self.use_fused_rotary = use_fused_rotary
...
#         print('using fused swiglu')
#         super().__init__()
#         # self.fc_1 = nn.Linear(config.n_embd, config.intermediate_size, bias=config.bias)
#         # self.fc_2 = nn.Linear(config.n_embd, config.intermediate_size, bias=config.bias)
#         # self.proj = nn.Linear(config.intermediate_size, config.n_embd, bias=config.bias)
#         self.swiglu = SwiGLU(config.hidden_size, config.intermediate_size, bias=False, _pack_weights=False)
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.is_causal = True
        self.use_fused_rotary = (config.rotary_type=="fused_rotary" and getattr(self.config, "_flash_attn_2_enabled", False))
    ...
    def _init_rope(self):
        if self.config.rope_scaling is None:
            self.rotary_emb = LlamaRotaryEmbedding(
                ...
                use_fused_rotary=self.use_fused_rotary
            )
        else:
    ...
    def forward(
        ...
        **kwargs,
        ...
        # if use fused rotary, shape of cos and sin need to be d/2
        if self.use_fused_rotary:
            raise Exception('fused rotary is not support in LlamaAttention')
        else:
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.use_fused_rotary:
            ...
            query_states = apply_rotary_emb(query_states, cos, sin, False, True)
            key_states = apply_rotary_emb(key_states, cos, sin, False, True)
