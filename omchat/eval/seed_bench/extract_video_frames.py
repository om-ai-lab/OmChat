...
dimension10_dir = (
    "/data3/ljj/proj/MLLM_evals/data/seed_bench/video/20bn-something-something-v2"
)
# root directory of evaluation dimension 11
dimension11_dir = "./videos/EPIC-KITCHENS"
# root directory of evaluation dimension 12
dimension12_dir = (
    "/data3/ljj/proj/MLLM_evals/data/seed_bench/video/BreakfastII_15fps_qvga_sync"
)
...
if __name__ == "__main__":
    data = json.load(open("/data3/ljj/proj/MLLM_evals/data/seed_bench/SEED-Bench.json"))
    video_img_dir = "/data3/ljj/proj/MLLM_evals/data/seed_bench/SEED-Bench-video-image"
    ques_type_id_to_name = {id: n for n, id in data["question_type"].items()}

    video_data = [x for x in data["questions"] if x["data_type"] == "video"]

    with open(
        "/data3/ljj/proj/MLLM_evals/data/seed_bench/executor.txt", "w"
    ) as f, concurrent.futures.ThreadPoolExecutor() as executor:
        future_to_images = {
